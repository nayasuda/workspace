...
describe('markdownToDocsRequests', () => {
  describe('parseMarkdownToDocsRequests', () => {
    ...
    it('should handle italic text with underscores', () => {
      ...
      expect(result.formattingRequests).toHaveLength(1);
      expect(
        result.formattingRequests[0].updateTextStyle?.textStyle?.italic,
      ).toBe(true);
    });
    ...
    it('should handle overlapping formatting (keeps first)', () => {
      ...
      expect(result.formattingRequests).toHaveLength(1);
      expect(
        result.formattingRequests[0].updateTextStyle?.textStyle?.bold,
      ).toBe(true);
    });
    ...
    it('should handle heading 1', () => {
      ...
      expect(result.formattingRequests).toHaveLength(1);
      expect(
        result.formattingRequests[0].updateParagraphStyle?.paragraphStyle
          ?.namedStyleType,
      ).toBe('HEADING_1');
      expect(
        result.formattingRequests[0].updateParagraphStyle?.range?.startIndex,
      ).toBe(0);
      expect(
        result.formattingRequests[0].updateParagraphStyle?.range?.endIndex,
      ).toBe(10);
    });
    ...
    it('should handle mixed headings and text', () => {
      ...
      const headingFormats = result.formattingRequests.filter(
        (req) =>
    ...
    it('should handle inline formatting within headings', () => {
      ...
      const headingFormat = result.formattingRequests.find(
        (req) =>
          req.updateParagraphStyle?.paragraphStyle?.namedStyleType !==
          undefined,
      );
      const boldFormat = result.formattingRequests.find(
